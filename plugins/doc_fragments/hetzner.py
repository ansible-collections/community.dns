...
options: {}
notes:
  - For C(CNAME) records, use absolute DNS names for values. Absolute DNS names are DNS names ending with
    a period C(.), for example C(foo.example.com.). If you use a relative DNS name, that is one without a
    trailing period, it will be treated relative to the zone of the C(CNAME) record.
'''

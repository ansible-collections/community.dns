...
class _Resolve:
    ...
    def _handle_reponse_errors(
        self, target, response, nameserver=None, query=None, accept_errors=None
    ):
        ...
        while True:
            try:
                response = self._handle_timeout(
                    resolver.resolve, dnsname, lifetime=self.timeout, **kwargs
                )
                ...
                try:
                    response = self._handle_timeout(
                        resolver.query, dnsname, lifetime=self.timeout, **kwargs
                    )
                except TypeError:
                    # For dnspython < 1.6.0
        ...
        **kwargs
        ...
        try:
            return self._resolve(
                resolver, dnsname, handle_response_errors=True, **kwargs
            )
...
class ResolveDirectlyFromNameServers(_Resolve):
    ...
    def _lookup_ns_names(self, target, nameservers=None, nameserver_ips=None):
        ...
        while True:
            response = self._handle_timeout(
                dns.query.udp, query, nameserver_ips[0], timeout=self.timeout
            )

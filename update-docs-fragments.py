...
def add_record_type_fragments(provider_fragment, provider_record_types):
    ...
    for fragment_name, insertion_point, doc_fragments in TYPE_DEPENDENT_FRAGMENTS:
        doc_fragments = [load_single_doc_fragment(doc_fragment) for doc_fragment in doc_fragments]
...
def main(program, arguments):
    ...
    try:
        ...
        print('{0}: Unexpected error: {1}'.format(program, e))
    finally:
        for error in errors:
            print(error)

...
def test_extract_error_message():
    ...
    assert (
        api._extract_error_message({"message": "foo"})
        == " with data: {'message': 'foo'}"
    )
...
def test_process_json_result():
    ...
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content=None, info={"status": 401, "url": "https://example.com"}
        )
    assert (
        exc.value.args[0]
        == "Unauthorized: the authentication parameters are incorrect (HTTP status 401)"
    )
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content='{"message": ""}'.encode("utf-8"),
            info={"status": 401, "url": "https://example.com"},
        )
    assert (
        exc.value.args[0]
        == "Unauthorized: the authentication parameters are incorrect (HTTP status 401)"
    ...
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content='{"message": "foo"}'.encode("utf-8"),
            info={"status": 401, "url": "https://example.com"},
        )
    assert (
        exc.value.args[0]
        == "Unauthorized: the authentication parameters are incorrect (HTTP status 401): foo"
    )
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content=None, info={"status": 403, "url": "https://example.com"}
        )
    assert (
        exc.value.args[0]
    ...
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content='{"message": ""}'.encode("utf-8"),
            info={"status": 403, "url": "https://example.com"},
        )
    assert (
        exc.value.args[0]
        == "Forbidden: you do not have access to this resource (HTTP status 403)"
    ...
    assert (
        exc.value.args[0]
        == "Forbidden: you do not have access to this resource (HTTP status 403): foo"
    )
    ...
    assert (
        exc.value.args[0]
        == "GET https://example.com did not yield JSON data, but HTTP status code 200 with data: not JSON"
    )

    info = {"status": 200, "url": "https://example.com"}
    info["Content-type"] = "application/json"
    r, i = api._process_json_result(
        content="not JSON".encode("utf-8"), info=info, must_have_content=False
    )
    assert r is None
    info = {"status": 200, "url": "https://example.com"}
    info["Content-type"] = "application/json"
    ...
    assert (
        exc.value.args[0]
        == "Expected successful HTTP status for GET https://example.com, but got HTTP status 404 (Not found) with data: {}"
    )
    ...
    with pytest.raises(DNSAPIError) as exc:
        api._process_json_result(
            content="{}".encode("utf-8"), info=info, expected=[200, 201]
        )
    assert (
        exc.value.args[0]
        == "Expected HTTP status 200, 201 for GET https://example.com, but got HTTP status 404 (Not found) with data: {}"
    )

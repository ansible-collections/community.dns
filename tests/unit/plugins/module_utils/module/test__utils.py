...
def test_get_prefix():
    ...
    assert get_prefix(
        normalized_zone="example.com",
        normalized_record="example.com",
        provider_information=provider_information,
    ) == ("example.com", None)
    assert get_prefix(
        normalized_zone="example.com",
        normalized_record="www.example.com",
        provider_information=provider_information,
    ) == ("www.example.com", "www")
    assert get_prefix(
        normalized_zone="example.com", provider_information=provider_information
    ) == ("example.com", None)
    assert get_prefix(
        normalized_zone="example.com",
        prefix="",
    ...
    with pytest.raises(DNSAPIError):
        get_prefix(
            normalized_zone="example.com",
            normalized_record="example.org",
            provider_information=provider_information,
        )
    with pytest.raises(DNSAPIError):
        get_prefix(
            normalized_zone="example.com",
            normalized_record="wwwexample.com",
            provider_information=provider_information,
        )

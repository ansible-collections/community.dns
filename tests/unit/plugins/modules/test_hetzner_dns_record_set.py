...
class TestHetznerDNSRecordNewJSON(BaseTestModule):
    ...
    def test_absent_check_mode(self, mocker):
        record = HETZNER_NEW_JSON_DEFAULT_ENTRIES[0]
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                mocker,
    ...
    def test_absent_via_empty(self, mocker):
        record = HETZNER_NEW_JSON_DEFAULT_ENTRIES[0]
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                mocker,
    ...
    def test_absent_via_empty_check_mode(self, mocker):
        record = HETZNER_NEW_JSON_DEFAULT_ENTRIES[0]
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                mocker,
        ...
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                ...
                [
                    ...
                    .return_header("Content-Type", "application/json")
                    .result_json(
                        get_hetzner_new_json_records(name="foobar", record_type="A")
                    ),
                ],
            )
        ...
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                ...
                [
                    ...
                    .expect_json_value(["records", 0, "comment"], None)
                    .expect_json_value(
                        ["records", 1, "value"], "ytterbium.ns.hetzner.com."
                    )
        ...
        with patch("time.sleep", mock_sleep):
            result = self.run_module_success(
                ...
                [
                    ...
                    .return_header("Content-Type", "application/json")
                    .result_json(
                        get_hetzner_new_json_records(name="foo", record_type="TXT")
                    ),
                    FetchUrlCall("POST", 201)
                    .expect_header("accept", "application/json")
                    .expect_header("Authorization", "Bearer foo")
                    .expect_url(
                        "https://api.hetzner.cloud/v1/zones/42/rrsets/foo/TXT/actions/set_records"
                    )
                    .expect_json_value(
                        ["records", 0, "value"],
                        '"bär \\"with quotes\\" (use \\\\ to escape)!"',
                    )
                    .expect_json_value(["records", 0, "comment"], None)
                    .expect_json_value_absent(["records", 1])
                    .return_header("Content-Type", "application/json")

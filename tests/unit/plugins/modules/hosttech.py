...
def validate_wsdl_call(conditions):
    def predicate(content):
        ...
        for header_ in root.iter(
            lxml.etree.QName("http://schemas.xmlsoap.org/soap/envelope/", "Header").text
        ):
            header = header_
        for body_ in root.iter(
            lxml.etree.QName("http://schemas.xmlsoap.org/soap/envelope/", "Body").text
        ):
            body = body_
...
def check_wsdl_nil(node):
    nil_flag = node.get(
        lxml.etree.QName("http://www.w3.org/2001/XMLSchema-instance", "nil")
    )
    if nil_flag != "true":
        print(nil_flag)  # pragma: no cover
...
def check_wsdl_value(node, value, wsdl_type=None):
    if type is not None:
        ...
        pass  # pragma: no cover
    if node.text != value:
        print(node.text, value)  # pragma: no cover
...
def find_xml_map_entry(map_root, key_name, allow_non_existing=False):
    for map_entry in map_root.iter("item"):
        ...
        if key.text == key_name:
            check_wsdl_value(
                key, key_name, wsdl_type=("http://www.w3.org/2001/XMLSchema", "string")
            )
            return value
...
def add_wsdl_answer_end_lines(lines):
    lines.extend(["</SOAP-ENV:Body>", "</SOAP-ENV:Envelope>"])
...
def check_wsdl_record(record_data, entry):
    check_wsdl_value(
        find_xml_map_entry(record_data, "type"),
        entry[2],
        wsdl_type=("http://www.w3.org/2001/XMLSchema", "string"),
    )
    prefix = find_xml_map_entry(record_data, "prefix")
    if entry[3]:
        check_wsdl_value(
            prefix, entry[3], wsdl_type=("http://www.w3.org/2001/XMLSchema", "string")
        )
        ...
        pass  # pragma: no cover
    check_wsdl_value(
        find_xml_map_entry(record_data, "target"),
        entry[4],
        wsdl_type=("http://www.w3.org/2001/XMLSchema", "string"),
    )
    check_wsdl_value(
        find_xml_map_entry(record_data, "ttl"),
        str(entry[5]),
        wsdl_type=("http://www.w3.org/2001/XMLSchema", "int"),
    )
    if entry[6] is None:
        ...
        check_wsdl_value(
            find_xml_map_entry(record_data, "comment"),
            entry[6],
            wsdl_type=("http://www.w3.org/2001/XMLSchema", "string"),
        )  # pragma: no cover
    if entry[7] is None:
        ...
        check_wsdl_value(
            find_xml_map_entry(record_data, "priority"),
            entry[7],
            wsdl_type=("http://www.w3.org/2001/XMLSchema", "string"),
        )  # pragma: no cover
...
def validate_wsdl_add_request(zone, entry):
    def predicate(content, header, body):
        fn_data = get_wsdl_value(
            body,
            lxml.etree.QName("https://ns1.hosttech.eu/public/api", "addRecord").text,
        )
        check_wsdl_value(
            get_wsdl_value(fn_data, "search"),
            zone,
            wsdl_type=("http://www.w3.org/2001/XMLSchema", "string"),
        )
...
def validate_wsdl_update_request(entry):
    def predicate(content, header, body):
        fn_data = get_wsdl_value(
            body,
            lxml.etree.QName("https://ns1.hosttech.eu/public/api", "updateRecord").text,
        )
        check_wsdl_value(
            get_wsdl_value(fn_data, "recordId"),
            str(entry[0]),
            wsdl_type=("http://www.w3.org/2001/XMLSchema", "int"),
        )
        check_wsdl_record(get_wsdl_value(fn_data, "recorddata"), entry)
...
def validate_wsdl_del_request(entry):
    def predicate(content, header, body):
        fn_data = get_wsdl_value(
            body,
            lxml.etree.QName("https://ns1.hosttech.eu/public/api", "deleteRecord").text,
        )
        check_wsdl_value(
            get_wsdl_value(fn_data, "recordId"),
            str(entry[0]),
...
HOSTTECH_WSDL_DEFAULT_ZONE_RESULT = create_wsdl_zones_answer(
    42, "example.com", HOSTTECH_WSDL_DEFAULT_ENTRIES
)
        ...
        "ds_records": [
            {
                ...
                "flags": 257,
                "protocol": 3,
                "public_key": "MuhdzsQdqEGShwjtJDKZZjdKqUSGluFzTTinpuEeIRzLLcgkwgAPKWFa "

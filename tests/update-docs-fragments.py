...
DEPENDENT_FRAGMENTS = [
    ...
    (
        ...
        [
            (
                "record_type",
                "options.record_sets.suboptions.type",
                [
                    "module_record_sets",
                ],
            ),
            (
                "record_default_ttl",
                "options.record_sets.suboptions.ttl",
                [
                    "module_record_sets",
                ],
            ),
    ...
    (
        ...
        [
            (
                "record_type",
                "options.simple_filters.suboptions.type",
                [
                    "inventory_records",
                ],
            ),
...
def get_provider_informations(providers):
    ...
    for provider in providers:
        ...
        try:
            ...
            errors.append(
                f"{full_py_path}: Cannot find function {create_provider_info_fn}"
            )
        except Exception as e:
            errors.append(
                f"{full_py_path}: Error while invoking function {create_provider_info_fn}: {e}"
            )
...
class DocFragment:
    ...
    def serialize_lines(self):
        return self.prefix_lines + [f'    {self.name} = r"""'] + self.lines + ['"""']
...
def augment_fragment(provider_fragment, provider_info):
    ...
    for fragment_name, fragment_insertion_data in DEPENDENT_FRAGMENTS:
        ...
        for what, insertion_point, doc_fragment_names in fragment_insertion_data:
            ...
            doc_fragments = [
                load_single_doc_fragment(doc_fragment)
                for doc_fragment in doc_fragment_names
            ]
        ...
        plural_s = "s" if len(all_doc_fragment_names) != 1 else ""
        fragment_names = ", ".join(sorted(all_doc_fragment_names))
        insertion_fragment.prefix_lines.extend(
            [
                f"    #          {line}"
                for line in textwrap.wrap(
                    f"It is used to augment the docs fragment{plural_s} {fragment_names}.",
                    width=80,
                )
            ]
        )
        ...
        for provider, provider_info in sorted(provider_infos.items()):
            try:
                ...
                if not compare_doc_fragment(provider, doc_fragment):
                    ...
                    if lint:
                        errors.append(
                            f"{path}: Needs to be updated by running 'nox -Re update-docs-fragments'"
                        )
                    else:
                ...
                errors.append(
                    f"{e.path}: Error while parsing docs fragment: {e.error_message}"
                )
